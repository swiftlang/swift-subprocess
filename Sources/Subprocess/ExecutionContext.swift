...
/// ``environment`` is `.inherit`, not the calling process's variables.
/// - The values the library resolved at launch time: ``resolvedExecutable``,
...
/// an error thrown before the process launched.
///
...
public struct ExecutionContext: Sendable, Hashable {
    ...
    /// if the subprocess was configured to inherit the calling process's
    /// working directory.
    ...
    /// The environment the subprocess was given, or `nil` if unavailable.
    ///
    /// This is `nil` for errors thrown before the process launched, and for
    ...
    /// `Subprocess` added so the subprocess can locate its DLLs, even when the
    /// caller did not supply one.
    public let resolvedEnvironment: [Environment.Key: String]?
    /// The subprocess's working directory.
    ///
    /// When the caller passed `nil`, this is the calling process's working
    /// directory captured at launch, or `nil` if undetermined.

        ...
        do {
            ...
            for try await _ in signalStream {
                ...
                while true {
                    let written: Int
                    do {
                        written = try span.withUnsafeBytes { ptr in
                            let remainingStart = ptr.baseAddress!.advanced(by: writtenLength)
                            let remainingCount = ptr.count - writtenLength
                            return try fileDescriptor.write(
                                UnsafeRawBufferPointer(start: remainingStart, count: remainingCount),
                                retryOnInterrupt: true
                            )
                        }
                    } catch {

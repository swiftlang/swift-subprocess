...
private nonisolated(unsafe) var _workerThread: Result<ThreadType, SubprocessError> = .failure(.spawnFailed)

private let _setupWorkerThread: () = {
    do throws(SubprocessError.UnderlyingError) {
        #if canImport(WinSDK)
        ...
        let subprocessError: SubprocessError = .spawnFailed(withUnderlyingError: error)

...
///
/// `maxRSS` is only asserted for children that demonstrably spend time on CPU.
/// FreeBSD samples RSS on statclock ticks instead of tracking a high-water mark
/// on every fault, so a child that sleeps, or dies immediately, is legitimately
/// accounted zero peak RSS. A child has to run to be measured.
extension SubprocessProcessMonitoringTests {
    ...
    ///
    /// The block is copied repeatedly rather than once so the child stays on CPU
    /// long enough to be sampled on kernels that record RSS per tick. `dd`
    /// reuses one buffer, so the peak stays at `bytes` regardless of `count`.
    private func allocatingProcess(bytes: Int) -> Configuration {
        return Configuration(
            executable: .path("/bin/dd"),
            arguments: ["if=/dev/zero", "of=/dev/null", "bs=\(bytes)", "count=8"]
        )
    }
    ...
    @Test func testResourceUsageChargesOnlyCPUTimeNotWallTime() async throws {
        ...
        #expect(result.terminationStatus.isSuccess)
        // `maxRSS` is deliberately not asserted here: this child is defined by
        // getting almost no CPU, and on a kernel that samples RSS per tick that
        // is the same thing as having no RSS sample. See the note above.
        #if !os(Windows)

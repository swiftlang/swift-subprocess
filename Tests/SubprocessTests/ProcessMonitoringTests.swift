...
struct SubprocessProcessMonitoringTests {
    ...
    private func longRunningProcess(withTimeOutSeconds timeout: Double? = nil) -> Configuration {
        ...
        #else
        let waitTime = timeout != nil ? "\(timeout!)" : "infinite"
        return Configuration(
...
extension SubprocessProcessMonitoringTests {
    @Test func testNormalExit() async throws {
        ...
        let monitorResult = try await monitorProcessTermination(
            for: spawnResult.execution.processIdentifier
        )
    ...
    @Test func testExitViaSignal() async throws {
        ...
        async let monitorResult = try monitorProcessTermination(
            for: spawnResult.execution.processIdentifier
        )
        // Send signal to process
        try spawnResult.execution.send(signal: .terminate)
...
extension SubprocessProcessMonitoringTests {
    @Test func testAlreadyTerminatedProcess() async throws {
        ...
        #if os(Windows)
        WaitForSingleObject(
            spawnResult.execution.processIdentifier.processDescriptor,
            INFINITE
        )
        #else
        ...
        waitid(
            P_PID,
            id_t(spawnResult.execution.processIdentifier.value),
            &siginfo,
            WEXITED | WNOWAIT
        )
        #endif
    ...
    @Test func testDoesNotReapUnrelatedChildProcess() async throws {
        ...
        let child1Result = try child1.spawn(
            withInput: self.devNullInputPipe(),
            outputPipe: self.devNullOutputPipe(),
            errorPipe: self.devNullOutputPipe()
        )
        defer {
            child1Result.execution.processIdentifier.close()
        }
        let child2 = self.immediateExitProcess(withExitCode: 0)
        let child2Result = try child2.spawn(
            withInput: self.devNullInputPipe(),
            outputPipe: self.devNullOutputPipe(),
            errorPipe: self.devNullOutputPipe()
        )
        defer {
            child2Result.execution.processIdentifier.close()
        }
        // Monitor child2, but make sure we don't reap child1's status
        let status = try await monitorProcessTermination(for: child2Result.execution.processIdentifier
        )
        #expect(status.isSuccess)
        // Make sure we can still fetch child 1
        #if os(Windows)
        let rc = WaitForSingleObject(
            child1Result.execution.processIdentifier.processDescriptor,
            INFINITE
        )
        ...
        let rc = waitid(
            P_PID,
            id_t(child1Result.execution.processIdentifier.value),
            &siginfo,
            WEXITED
        )
        #expect(rc == 0)
